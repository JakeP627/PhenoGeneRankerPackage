...
create.bipartite.matrix <- function(WholeNet, N, M, gene_pool_nodes_sorted, phenotype_pool_nodes_sorted, 
                                    numCores) {
    # WholeNet <- FullNet
    ...
    # if (!isWeighted){ value <- rep(1, nrow(el)) }
...
create.transition.matrix <- function(SupraBipartiteMatrix, N, M, LG, LP, lambda, 
                                     isTranspose) {
...
create.transition.matrix.gene_pheno <- function(SupraBipartiteMatrix, N, M, LG, LP, 
                                                lambda) {
    #### Transition Matrix for the inter-subnetworks links
...
create.transition.matrix.pheno_gene <- function(SupraBipartiteMatrix, N, M, LG, LP, 
                                                lambda) {
...
create.transition.multiplex.network <- function(SupraAdjacencyMatrix, SupraBipartiteMatrix, 
...
create.gene.transition.multiplex.network <- function(SupraAdjacencyMatrix, SupraBipartiteMatrix, 
...
create.pheno.transition.multiplex.network <- function(SupraAdjacencyMatrixPheno, 
...
rank_phenotypes <- function(Number_Genes, Num_Gene_Layers, Number_Phenotypes, Num_Pheno_Layers, 
                            Results, Seeds) {
    ...
    phenotypes_rank$Score <- geometric.mean(as.vector(Results[, 1])[(Number_Genes * 
...
#' all of the genes ranked. It has a column of the rank, gene name, and and steady state scores for each gene or phenotype.
#'
#' @examples
...
RandomWalkRestarts <- function(walk_Matrix, geneSeeds, phenoSeeds, generatePValue = TRUE, 
    ...
    Seeds_Score <- get.seed.scores(SeedList[["Genes_Seeds"]], SeedList[["Pheno_Seeds"]], 
    ...
    RWGeneRankDF <- rank_genes(walk_Matrix[["N"]], walk_Matrix[["LG"]], prox_vector, 
                               SeedList[["Genes_Seeds"]])
    RWPhenoRankDF <- rank_phenotypes(walk_Matrix[["N"]], walk_Matrix[["LG"]], walk_Matrix[["M"]], 
                                     walk_Matrix[["LP"]], prox_vector, SeedList[["Pheno_Seeds"]])
    
    if (generatePValue) {
        # Generate random seeds
        RandomSeeds <- generate.random.seed.vector(walk_Matrix, SeedList[["Genes_Seeds"]], 
        ...
        # categoraiz ethe results for candidate genes dfRank <-
        # categorize.ranked.genes(CandidateGenes, dfRank)
...
Random_Walk_Restarts_Single <- function(Walk_Matrix, r, Seeds_Score) {
    ...
    ### walker. Seeds_Score <- get.seed.scores(geneSeeds,CultSeeds, Parameters$eta, LG,
...
get.WalkMatrixID <- function(filesDF) {
    WM_DF <- read.table(filesDF$file_name[filesDF$layer_type == "WM_DF"], header = TRUE, 
                        sep = "\t", stringsAsFactors = FALSE)
    LayerNames <- filesDF$layer_name[filesDF$type %in% c("gene", "pheno", "bipartite")]
    LayerNames <- paste0(LayerNames, collapse = ",")
    new_WM <- data.frame(ID = ifelse(length(WM_DF$ID) == 0, 1, max(WM_DF$ID) + 1), 
                         Layers = LayerNames, Date = as.character(Sys.time()))
    WM_DF <- rbind(WM_DF, new_WM)
    res <- try(write.table(WM_DF, filesDF$file_name[filesDF$layer_type == "WM_DF"], 
                           sep = "\t", col.names = TRUE, row.names = FALSE, quote = FALSE))
    if (!is.null(res)) 
        stop("Eror: Could not get Walk Matrix ID!")
    output = list(ID = max(WM_DF$ID), LayerNames = LayerNames)
    return(output)
}
...
#'
#' @return This returns a list that contains a walk matrix, list of sorted genes by name in the multiplex heterogeneous network, list of phenotypes sorted by name in the multiplex heterogeneous networkpool nodes, the degree of all genes in the multiplex heterogeneous network to be used for p-value generation in randomWalkRestarts method, the degree of all genes/phenotypes in the multiplex heterogeneous network to be used for p-value generation in randomWalkRestarts method, the number of gene layer files there are, the number of phenotype layer files there are, the number of gene pool nodes, and the number of phenotype nodes.
#' 
#' @export
#'
#' @examples
...
CreateWalkMatrix <- function(inputFileName, numCores = 1, delta = 0.5, zeta = 0.5, 
    ...
    # cat('Creating Walk Matrix For: ',
    # paste0(FilesDF$layer_name[FilesDF$type%in%c('gene', 'pheno', 'bipartite')],
    # collapse = ','), '\n')
    ...
    SupraAdjacencyMatrix <- create.supraadjacency.matrix(FullNet, "gene", N, LG, 
    ...
    SupraAdjacencyMatrixPheno <- create.supraadjacency.matrix(FullNet, "phenotype", 
                                                              M, LP, Parameters$delta, TRUE)
    # cat('Time to create SupraAdjacencyMatrix for Phenoivars : ',
    # format(Sys.time()-t1), '\n')
    
    # CREATE THE BIPARTITE GRAPH t1 <- Sys.time()
    BipartiteMatrix <- create.bipartite.matrix(FullNet, N, M, gene_pool_nodes_sorted, 
    ...
    # t1 <- Sys.time()
    SupraBipartiteMatrix <- create.suprabipartite.matrix(BipartiteMatrix, N, M, LG, 
                                                         LP)
    # cat('Time to create SupraBipartiteMatrix : ', format(Sys.time()-t1), '\n')
    
    
    # t1 <- Sys.time() Transition_Gene_Phenoivar <-
    # create.transition.matrix.gene_pheno(SupraBipartiteMatrix, N, M, LG, LP,
    # Parameters$lambda)
    Transition_Gene_Phenoivar <- create.transition.matrix(SupraBipartiteMatrix, N, 
                                                          M, LG, LP, Parameters$lambda, FALSE)
    # cat('Time to create Transition Matrix for Genes-Phenoivars : ',
    # format(Sys.time()-t1), '\n')
    
    # t1 <- Sys.time() Transition_Phenoivar_Gene <-
    # create.transition.matrix.pheno_gene(SupraBipartiteMatrix, N, M, LG, LP,
    # Parameters$lambda)
    Transition_Phenoivar_Gene <- create.transition.matrix(SupraBipartiteMatrix, N, 
    ...
    # create.transition.multiplex.network(SupraAdjacencyMatrix, SupraBipartiteMatrix,
    # N, LG, Parameters$lambda, numCores) cat('Time to create Gene Transition
    # Multiplex Network : ', format(Sys.time()-t1), '\n')
    ...
    # create.transition.multiplex.network(SupraAdjacencyMatrixPheno,
    # SupraBipartiteMatrix, M, LP, Parameters$lambda, numCores) cat('Time to create
    # Pheno Transition Multiplex Network : ', format(Sys.time()-t1), '\n')
...
generate.random.seeds <- function(Seeds, ConnectivityDF, S = 1000, no.groups = 10, 
                                  replace = FALSE) {
    ...
    RandomSeeds <- ConnectivityDF[which(!ConnectivityDF$Node %in% Seeds), ] %>% group_by(Group) %>% 
...
generate.random.seed.vector <- function(WM, geneSeeds, phenoSeeds, S = 1000, no.groups.gene = 10, 
                                        no.groups.pheno = 5) {
    
    if (length(geneSeeds) == 0 && length(phenoSeeds) == 0) 
        stop("No seeds provided!")
    GeneConnectivity <- assign.group.to.connectivityDF(WM[["gene_connectivity"]], 
                                                       no.groups = no.groups.gene)
    PhenoConnectivity <- assign.group.to.connectivityDF(WM[["phenotype_connectivity"]], 
    ...
    if (length(geneSeeds) != 0) {
        RandomgeneSeeds <- generate.random.seeds(geneSeeds, GeneConnectivity, S, 
                                                 no.groups.gene, TRUE)
    ...
    if (length(phenoSeeds) != 0) {
        RandomphenoSeeds <- generate.random.seeds(phenoSeeds, PhenoConnectivity, 
    ...
    # if(length(geneSeeds)!=0 & length(phenoSeeds)!=0){ RandomSeeds <- mapply(c,
    # RandomgeneSeeds, RandomphenoSeeds, SIMPLIFY=FALSE) }else
    # if(length(RandomSeeds)==0) stop('No seeds provided!')
...
calculatePvalues <- function(RWGeneRanks, Rand_Seed_Gene_Rank, no.cores) {
    ...
    for (i in 1:length(rank.offset)) {
        dfRanks$P <- base::rowMeans(dfRanks[, 3:(S + 2)] < (dfRanks[, S + 2 + i]), 
                                    na.rm = TRUE)
        
        # rename column P by using offset value e.g. as P10, P20 ...
        dfRanks <- dplyr::rename(dfRanks, `:=`((!!as.name(paste0("P", rank.offset[i]))), 
                                               P))
    }
    ...
    Rand_Seed_Gene_Rank <- foreach(i = 1:seedsLength, .combine = cbind, .export = funcs, 
                                   .packages = c("Matrix")) %dopar% {
                                       ...
                                       if (length(geneSeedsList) != 0 && length(phenoSeedsList) != 0) {
                                           Seeds_Score <- get.seed.scores(geneSeedsList[[i]], phenoSeedsList[[i]], 
                                                                          eta, LG, LP, tau, phi)
                                       } else if (length(geneSeedsList) != 0) {
                                           Seeds_Score <- get.seed.scores(geneSeedsList[[i]], vector(), eta, LG, 
                                                                          LP, tau, phi)
                                       } else {
                                           Seeds_Score <- get.seed.scores(vector(), phenoSeedsList[[i]], eta, LG, 
                                                                          LP, tau, phi)
                                       }
                                       
                                       Rand_Seed_Res <- Random_Walk_Restarts_Single(Walk_Matrix, r, Seeds_Score)
                                       Rand_Seed_Gene_Rank <- rank_genes(N, LG, Rand_Seed_Res, ifelse(length(geneSeedsList) != 
